...
import zve.com.vn.dto.yeucaunvl.YeucauNvlItemImpl;
...
public class YeuCauNVLItemRepositoryImpl implements YeuCauNVLItemRepositoryCustom {
	...
	/* ---------------------------------------------------------- */
	@Override
	public List<YeucauNvlItemProjection> findAllItems(String model, Integer plan) {
		   String sql = """
		   ...
		   BigDecimal planDecimal = BigDecimal.valueOf(plan);
		   ...
		   return secondJdbcTemplate.query(
				    ...
				    (rs, rowNum) -> {
				        ...
				        BigDecimal planResult = (psQtyPer != null) ? psQtyPer.multiply(planValue) : BigDecimal.ZERO;
				        planResult = planResult.setScale(3, RoundingMode.HALF_UP);  // giữ 3 chữ số sau dấu phẩy

				        
				        return new YeucauNvlItemImpl(
